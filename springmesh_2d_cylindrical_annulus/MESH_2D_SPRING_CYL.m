function MESH_2D_SPRING_CYL
% MESH_2D_SPRING_CYL - 2D FINITE ELEMENT CYLINDRICAL ANNULUS MESH GENERATOR
% 
...
%
% Basic idea:
%   -- A MATLAB based 2D unstructured triangular cylindrical mesh generator
...
%     -> The guide-mesh is defined in polar coordinates since the  
...
%     -> Boundary nodes are fixed on the circular boundary using projection
%        method (can only move along the tangent line and then they are
%        pulled back towards the circle by projecting into the radial
%        direction).
...
%
% JMT Jun 2015
% JMT May 2016 : cleaned up and made it compatible with M2TRI_CYL
...
fprintf(1,'\n\n\n\n');
fprintf(1,'========================================================\n');
fprintf(1,'        NEW CALCULATION USING MESH_2D_SPRING_CYL\n');
fprintf(1,'========================================================\n');
addpath([pwd '/mfiles_CYL_MESH']); % MESH_2D_SPRING_CYL core functions
...
while(MESH.iter <= SETTINGS.itmax &&...
    ...
    if MESH.mean_misfit_bar_length >= SETTINGS.mean_misfit_bar_length_tol
        ...
        %==================================================================
        if abs(rms(MESH.rho)-rms(MESH.rho0))/rms(MESH.rho) > 1
            %==============================================================
...
end % END OF FUNCTION MESH_2D_SPRING_CYL