function plot_misfit_bar_length_to_check_nodal_density(MESH,SETTINGS)
% Usage: plot_misfit_bar_length_to_check_nodal_density(MESH,SETTINGS)
%
% Purpose:
...
title('Misfit bar-length to check nodal density')
xlabel('% of misfit')
ylabel('% of bars')
...
if SETTINGS.save_figs
    ...
    switch SETTINGS.fig_type
        case 'png'
            print('-dpng','-r150',[prefix 'Misfit_bar_length_to_check_nodal_density' suffix]);
        case 'fig'
            saveas(gcf,[prefix 'Misfit_bar_length_to_check_nodal_density' suffix],'fig');
        otherwise
            error(' Invalid value of "SETTINGS.fig_type".');
    end
end

end % END OF FUNCTION plot_misfit_bar_length_to_check_nodal_density