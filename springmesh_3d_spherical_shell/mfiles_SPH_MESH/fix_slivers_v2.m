function MESH = fix_slivers_v2(MESH,GUIDE_MESH,INTERFACE,SETTINGS)
% Usage: MESH = fix_slivers_v2(MESH,GUIDE_MESH,INTERFACE,SETTINGS)
%
% Purpose:
...
warning('fix_sliver_v2.m may not fully work (it needs to be debugged)')
...
bnd1_nodes_to_remove             = []; % empty bnd1_nodes_to_remove to enter in the loop
bnd2_nodes_to_remove             = []; % empty bnd2_nodes_to_remove to enter in the loop
bnd_ref_face_bot_nodes_to_remove = []; % empty bnd_ref_face_bot_nodes_to_remove just to enter into the loop
bnd_ref_face_1_nodes_to_remove   = []; % empty bnd_ref_face_1_nodes_to_remove just to enter into the loop
bnd_ref_face_2_nodes_to_remove   = []; % empty bnd_ref_face_2_nodes_to_remove just to enter into the loop
bnd_ref_face_3_nodes_to_remove   = []; % empty bnd_ref_face_3_nodes_to_remove just to enter into the loop
bnd_ref_face_4_nodes_to_remove   = []; % empty bnd_ref_face_4_nodes_to_remove just to enter into the loop
int_nodes_to_remove              = []; % empty int_nodes_to_remove to enter in the loop
...
if nfix > 12
    ...
    bars_all_slivers(sum(ismember(bars_all_slivers,(1:nfix)),2)==2,:) = []; % remove bars formed only by fixed nodes
end
...
for i = 1:size(SLIVER2NOD_sorted,1)
    ...
    num_bnd_nodes = sum( sorted_nodes > nfix & sorted_nodes <= nfix              + ...
                                                               nbnd1             + ...
                                                               nbnd2             + ...
                                                               nbnd_ref_face_bot + ...
                                                               nbnd_ref_face_1   + ...
                                                               nbnd_ref_face_2   + ...
                                                               nbnd_ref_face_3   + ...
                                                               nbnd_ref_face_4); % number of bnd nodes for this sliver
%     % check if the bnd nodes are on the same bnd (the routine only works with bnd nodes on the same bnd)
%     i_bnd = sorted_nodes > nfix & sorted_nodes <= nfix              + ...
%                                                   nbnd1             + ...
%                                                   nbnd2             + ...
%                                                   nbnd_ref_face_bot + ...
%                                                   nbnd_ref_face_1   + ...
...
%                                                   nbnd_ref_face_3   + ...
%                                                   nbnd_ref_face_4;
%     n_bnd1             = sum(ismember(sorted_nodes(i_bnd),nfix+1:nfix+nbnd1));
%     n_bnd2             = sum(ismember(sorted_nodes(i_bnd),nfix+nbnd1+1:nfix+nbnd1+nbnd2));
%     n_bnd_ref_face_bot = sum(ismember(sorted_nodes(i_bnd),nfix+nbnd1+nbnd2+1:nfix+nbnd1+nbnd2+nbnd_ref_face_bot));
%     n_bnd_ref_face_1   = sum(ismember(sorted_nodes(i_bnd),nfix+nbnd1+nbnd2+nbnd_ref_face_bot+1:nfix+nbnd1+nbnd2+nbnd_ref_face_bot+nbnd_ref_face_1));
%     n_bnd_ref_face_2   = sum(ismember(sorted_nodes(i_bnd),nfix+nbnd1+nbnd2+nbnd_ref_face_bot+nbnd_ref_face_1+1:nfix+nbnd1+nbnd2+nbnd_ref_face_bot+nbnd_ref_face_1+nbnd_ref_face_2));
%     n_bnd_ref_face_3   = sum(ismember(sorted_nodes(i_bnd),nfix+nbnd1+nbnd2+nbnd_ref_face_bot+nbnd_ref_face_1+nbnd_ref_face_2+1:nfix+nbnd1+nbnd2+nbnd_ref_face_bot+nbnd_ref_face_1+nbnd_ref_face_2+nbnd_ref_face_3));
%     n_bnd_ref_face_4   = sum(ismember(sorted_nodes(i_bnd),nfix+nbnd1+nbnd2+nbnd_ref_face_bot+nbnd_ref_face_1+nbnd_ref_face_2+nbnd_ref_face_3+1:nfix+nbnd1+nbnd2+nbnd_ref_face_bot+nbnd_ref_face_1+nbnd_ref_face_2+nbnd_ref_face_3+nbnd_ref_face_4));
%     if n_bnd1             == num_bnd_nodes || ...
%             n_bnd2             == num_bnd_nodes || ...
%             n_bnd_ref_face_bot == num_bnd_nodes || ...
%             n_bnd_ref_face_1   == num_bnd_nodes || ...
%             n_bnd_ref_face_2   == num_bnd_nodes || ...
%             n_bnd_ref_face_3   == num_bnd_nodes || ...
%             n_bnd_ref_face_4   == num_bnd_nodes
%         % bnd nodes are on the same bnd
%     else
%         % pick the case with more bnd nodes
%         num_bnd_nodes = max([n_bnd1 n_bnd2 n_bnd_ref_face_bot n_bnd_ref_face_1 n_bnd_ref_face_2 n_bnd_ref_face_3 n_bnd_ref_face_4]);
%     end
    
    if r_int > 0 && ~isempty(pbnd1)
        I_bnd1    = find(sorted_nodes >  nfix & sorted_nodes <= nfix + nbnd1); % local index of bnd1 nodes for this sliver
    else
        I_bnd1    = [];
    end
    I_bnd2             = find(sorted_nodes >  nfix + nbnd1 &...
                              sorted_nodes <= nfix + nbnd1 + nbnd2); % local index of bnd2 nodes for this sliver
    I_bnd_ref_face_bot = find(sorted_nodes >  nfix + nbnd1 + nbnd2 & ...
                              sorted_nodes <= nfix + nbnd1 + nbnd2 + nbnd_ref_face_bot); % local index of bnd_ref_face_bot nodes for this sliver
    I_bnd_ref_face_1   = find(sorted_nodes >  nfix + nbnd1 + nbnd2 + nbnd_ref_face_bot & ...
                              sorted_nodes <= nfix + nbnd1 + nbnd2 + nbnd_ref_face_bot + nbnd_ref_face_1); % local index of bnd_ref_face_1 nodes for this sliver
    I_bnd_ref_face_2   = find(sorted_nodes >  nfix + nbnd1 + nbnd2 + nbnd_ref_face_bot + nbnd_ref_face_1 & ...
                              sorted_nodes <= nfix + nbnd1 + nbnd2 + nbnd_ref_face_bot + nbnd_ref_face_1 + nbnd_ref_face_2); % local index of bnd_ref_face_2 nodes for this sliver
    I_bnd_ref_face_3   = find(sorted_nodes >  nfix + nbnd1 + nbnd2 + nbnd_ref_face_bot + nbnd_ref_face_1 + nbnd_ref_face_2 & ...
                              sorted_nodes <= nfix + nbnd1 + nbnd2 + nbnd_ref_face_bot + nbnd_ref_face_1 + nbnd_ref_face_2 + nbnd_ref_face_3); % local index of bnd_ref_face_3 nodes for this sliver
    I_bnd_ref_face_4   = find(sorted_nodes >  nfix + nbnd1 + nbnd2 + nbnd_ref_face_bot + nbnd_ref_face_1 + nbnd_ref_face_2 + nbnd_ref_face_3 & ...
                              sorted_nodes <= nfix + nbnd1 + nbnd2 + nbnd_ref_face_bot + nbnd_ref_face_1 + nbnd_ref_face_2 + nbnd_ref_face_3 + nbnd_ref_face_4); % local index of bnd_ref_face_4 nodes for this sliver
    I_int              = find(sorted_nodes >  nfix + nbnd1 + nbnd2 + nbnd_ref_face_bot + nbnd_ref_face_1 + nbnd_ref_face_2 + nbnd_ref_face_3 + nbnd_ref_face_4); % local index of int nodes for this sliver
    ...
    check_fixed_sliver = sum(ismember(sorted_nodes_no_fixed_nodes', [bnd1_nodes_to_remove;             ...
                                                                     bnd2_nodes_to_remove;             ...
                                                                     bnd_ref_face_bot_nodes_to_remove; ...
                                                                     bnd_ref_face_1_nodes_to_remove;   ...
                                                                     bnd_ref_face_2_nodes_to_remove;   ...
                                                                     bnd_ref_face_3_nodes_to_remove;   ...
                                                                     bnd_ref_face_4_nodes_to_remove;   ...
                                                                     int_nodes_to_remove])) == 0;
    if check_fixed_sliver % check_fixed_sliver = 1 --> no node has been removed
        ...
        elseif num_bnd_nodes == 1 % boundary sliver with 1 bnd node
            ...
            elseif ~isempty(I_bnd2)             % sliver on boundary 2
            ...
            elseif ~isempty(I_bnd_ref_face_bot) % sliver on bottom face of the refined region
                [bnd_ref_face_bot_nodes_to_remove,int_nodes_to_remove,pint_new] = ...
                    compare_q_slivers_1bnd_node(GCOORD_tri_1,q1,q2,q3,sorted_nodes,bnd_ref_face_bot_nodes_to_remove,int_nodes_to_remove,pint_new,shared_bars,barmid,bars,I_fix,I_bnd_ref_face_bot,I_int);
            elseif ~isempty(I_bnd_ref_face_1)   % sliver face 1 of the refined region
                [bnd_ref_face_1_nodes_to_remove,int_nodes_to_remove,pint_new] = ...
                    compare_q_slivers_1bnd_node(GCOORD_tri_1,q1,q2,q3,sorted_nodes,bnd_ref_face_1_nodes_to_remove,int_nodes_to_remove,pint_new,shared_bars,barmid,bars,I_fix,I_bnd_ref_face_1,I_int);
            elseif ~isempty(I_bnd_ref_face_2)   % sliver face 2 of the refined region
                [bnd_ref_face_2_nodes_to_remove,int_nodes_to_remove,pint_new] = ...
                    compare_q_slivers_1bnd_node(GCOORD_tri_1,q1,q2,q3,sorted_nodes,bnd_ref_face_2_nodes_to_remove,int_nodes_to_remove,pint_new,shared_bars,barmid,bars,I_fix,I_bnd_ref_face_2,I_int);
            elseif ~isempty(I_bnd_ref_face_3)   % sliver face 3 of the refined region
                [bnd_ref_face_3_nodes_to_remove,int_nodes_to_remove,pint_new] = ...
                    compare_q_slivers_1bnd_node(GCOORD_tri_1,q1,q2,q3,sorted_nodes,bnd_ref_face_3_nodes_to_remove,int_nodes_to_remove,pint_new,shared_bars,barmid,bars,I_fix,I_bnd_ref_face_3,I_int);
            elseif ~isempty(I_bnd_ref_face_4)   % sliver face 4 of the refined region
                [bnd_ref_face_4_nodes_to_remove,int_nodes_to_remove,pint_new] = ...
                    compare_q_slivers_1bnd_node(GCOORD_tri_1,q1,q2,q3,sorted_nodes,bnd_ref_face_4_nodes_to_remove,int_nodes_to_remove,pint_new,shared_bars,barmid,bars,I_fix,I_bnd_ref_face_4,I_int);
            end
            
        elseif num_bnd_nodes == 2 && num_shared_bars == 0 % boundary sliver with 2 bnd nodes but no shared bars
            %% NEED TO CREATE THE CASES FOR BND NODES IN DIFFERENT BOUNDARIES (THIS ROUTINE ASSUMES 2 BND NODES ON THE SAME BOUNDARY)
            ...
            elseif ~isempty(I_bnd_ref_face_bot) % sliver on bottom face of the refined region
                [bnd_ref_face_bot_nodes_to_remove,int_nodes_to_remove,pbnd_ref_face_bot_new,pint_new] = ....
                    compare_q_slivers_2bnd_nodes(GCOORD,GCOORD_tri_1,q1,q2,sorted_nodes,bnd_ref_face_bot_nodes_to_remove,int_nodes_to_remove,pbnd_ref_face_bot_new,pint_new,I_fix,I_bnd_ref_face_bot,I_int);
            elseif ~isempty(I_bnd_ref_face_1)   % sliver on face 1 of the refined region
                [bnd_ref_face_1_nodes_to_remove,int_nodes_to_remove,pbnd_ref_face_1_new,pint_new] = ....
                    compare_q_slivers_2bnd_nodes(GCOORD,GCOORD_tri_1,q1,q2,sorted_nodes,bnd_ref_face_1_nodes_to_remove,int_nodes_to_remove,pbnd_ref_face_1_new,pint_new,I_fix,I_bnd_ref_face_1,I_int);
            elseif ~isempty(I_bnd_ref_face_2)   % sliver on face 2 of the refined region
                [bnd_ref_face_2_nodes_to_remove,int_nodes_to_remove,pbnd_ref_face_2_new,pint_new] = ....
                    compare_q_slivers_2bnd_nodes(GCOORD,GCOORD_tri_1,q1,q2,sorted_nodes,bnd_ref_face_2_nodes_to_remove,int_nodes_to_remove,pbnd_ref_face_2_new,pint_new,I_fix,I_bnd_ref_face_2,I_int);
            elseif ~isempty(I_bnd_ref_face_3)   % sliver on face 3 of the refined region
                [bnd_ref_face_3_nodes_to_remove,int_nodes_to_remove,pbnd_ref_face_3_new,pint_new] = ....
                    compare_q_slivers_2bnd_nodes(GCOORD,GCOORD_tri_1,q1,q2,sorted_nodes,bnd_ref_face_3_nodes_to_remove,int_nodes_to_remove,pbnd_ref_face_3_new,pint_new,I_fix,I_bnd_ref_face_3,I_int);
            elseif ~isempty(I_bnd_ref_face_4)   % sliver on face 4 of the refined region
                [bnd_ref_face_4_nodes_to_remove,int_nodes_to_remove,pbnd_ref_face_4_new,pint_new] = ....
                    compare_q_slivers_2bnd_nodes(GCOORD,GCOORD_tri_1,q1,q2,sorted_nodes,bnd_ref_face_4_nodes_to_remove,int_nodes_to_remove,pbnd_ref_face_4_new,pint_new,I_fix,I_bnd_ref_face_4,I_int);
            end
...
if ~isempty(pbnd2_new) && ~isempty(r_ext)
...
pbnd_ref_face_bot(bnd_ref_face_bot_nodes_to_remove-nfix-nbnd1-nbnd2,:) = []; % Remove bnd_ref_face_bot nodes
...
pbnd_ref_face_bot = [pbnd_ref_face_bot; pbnd_ref_face_bot_new]; % Add new bnd_ref_face_bot points

% REFINE REGION FACE 1
pbnd_ref_face_1(bnd_ref_face_1_nodes_to_remove-nfix-nbnd1-nbnd2-nbnd_ref_face_bot,:) = []; % Remove bnd_ref_face_1 nodes
pbnd_ref_face_1   = [pbnd_ref_face_1; pbnd_ref_face_1_new]; % Add new bnd_ref_face_1 points

% REFINE REGION FACE 2
pbnd_ref_face_2(bnd_ref_face_2_nodes_to_remove-nfix-nbnd1-nbnd2-nbnd_ref_face_bot-nbnd_ref_face_1,:) = []; % Remove bnd_ref_face_2 nodes
pbnd_ref_face_2   = [pbnd_ref_face_2; pbnd_ref_face_2_new]; % Add new bnd_ref_face_2 points

% REFINE REGION FACE 3
pbnd_ref_face_3(bnd_ref_face_3_nodes_to_remove-nfix-nbnd1-nbnd2-nbnd_ref_face_bot-nbnd_ref_face_1-nbnd_ref_face_2,:) = []; % Remove bnd_ref_face_3 nodes
pbnd_ref_face_3   = [pbnd_ref_face_3; pbnd_ref_face_3_new]; % Add new bnd_ref_face_3 points

% REFINE REGION FACE 4
pbnd_ref_face_4(bnd_ref_face_4_nodes_to_remove-nfix-nbnd1-nbnd2-nbnd_ref_face_bot-nbnd_ref_face_1-nbnd_ref_face_2-nbnd_ref_face_3,:) = []; % Remove bnd_ref_face_4 nodes
pbnd_ref_face_4   = [pbnd_ref_face_4; pbnd_ref_face_4_new]; % Add new bnd_ref_face_4 points

% INTERIOR
pint(int_nodes_to_remove-nfix-nbnd1-nbnd2-nbnd_ref_face_bot-nbnd_ref_face_1-nbnd_ref_face_2-nbnd_ref_face_3-nbnd_ref_face_4,:) = [];  % Remove interior nodes
...
if r_int > 0
    ...
    EL2NOD_new       = EL2NOD_new(~(sum(ismember(EL2NOD_new,find(abs(GCOORD_SPH_new(:,3)-r_int) < 1e-8)),2)==4),:);
end
q_new            = tetra_mesh_quality(GCOORD_new,EL2NOD_new);
new_slivers      = EL2NOD_new(q_new < q_sliver,:);
sliver_nodes     = SLIVER2NOD_sorted(:);
sliver_bnd_nodes = sliver_nodes(sliver_nodes<=nfix+nbnd1+nbnd2+nbnd_ref_face_bot+nbnd_ref_face_1+nbnd_ref_face_2+nbnd_ref_face_3+nbnd_ref_face_4);
...
end % END OF FUNCTION fix_slivers_v2
