function plot_percentage_q_factor(q,iter,SETTINGS,s)
% Usage: plot_percentage_q_factor(q,iter,SETTINGS,s)
%
% Purpose:
...
%   s        : [column vector] : shape factor for each tetrahedron 
%
% Output:
%   plot
%
...
if nargin == 4
    [ns,~] = compute_pct(s);
    plot(x,ns,'b')
    legend('quality factor, q','tetrahedral shape, s')
end
title('Percentage of tetrahedra with a given quality factor')
xlabel('quality factor')
ylabel('% of tetrahedra')
